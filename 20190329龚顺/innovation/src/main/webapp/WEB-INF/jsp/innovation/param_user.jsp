<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jstl/fmt" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ page isELIgnored="false" %>
<c:set value="${pageContext.request.contextPath}" var="path" scope="page"/>
<!DOCTYPE html>
<html>
<head>
    <title>Title</title>
    <jsp:include page="../vuetemplate/css_vue_template.jsp" flush="true"></jsp:include>
    <link type="text/css" href="${path}/static/css/jsp/param_user.css"/>
</head>
<body>

<div id="app">

    <el-row :gutter="10">
        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
            <el-form :inline="true" :model="formInline" class="demo-form-inline">
                <el-form-item label="用户身份：">
                    <el-select v-model="formInline.region" placeholder="全部">
                        <el-option label="全部" value="0" selected></el-option>
                        <el-option label="学生" value="1"></el-option>
                        <el-option label="老师" value="2"></el-option>
                        <el-option label="专家" value="3"></el-option>
                        <el-option label="管理员" value="4"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" @click="onSubmitByEx">查询</el-button>
                </el-form-item>
            </el-form>
        </el-col>
        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
            <template>
                <el-table :data="tableData" style="width: 100%">
                    <el-table-column prop="num" label="编号"></el-table-column>
                    <el-table-column prop="name" label="账号"></el-table-column>
                    <el-table-column prop="realName" label="姓名"></el-table-column>
                    <el-table-column prop="sex" :formatter="sexFormatter" label="性别"></el-table-column>
                    <el-table-column prop="tel" label="电话"></el-table-column>
                    <el-table-column prop="descInfo" label="简介"></el-table-column>
                    <el-table-column prop="level" :formatter="levelFormatter" label="身份"></el-table-column>
                    <el-table-column label="操作">
                        <template slot-scope="scope">
                            <el-button size="mini" @click="handleDelete(scope.$index, scope.row)">修改</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </template>
        </el-col>
    </el-row>
    <el-row :gutter="10">
        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" style="text-align: center">
            <el-pagination
                    @size-change="handleSizeChange"
                    @current-change="handleCurrentChange"
                    :current-page="pageInfo.pageNum"
                    :page-size="pageInfo.pageSize"
                    layout="total, prev, pager, next"
                    :total="pageInfo.total"
                    background
            >
            </el-pagination>
        </el-col>
    </el-row>

    <%--弹出框（详细）--%>
    <template>
        <el-dialog title="" :visible.sync="outerVisible" class="customClass" top="true" width="80%" center>
            <div slot="title" class="dialog-header">
                显示详细
            </div>

            <el-form ref="tableFormRow" label-width="80px" v-model="tableDataRow" size="mini">
                <el-form-item label="项目名称">
                    {{tableDataRow.name}}
                </el-form-item>
                <el-form-item label="项目描述">
                    {{tableDataRow.descInfo}}
                </el-form-item>
                <el-form-item label="附件文档" v-if="tableDataRow.attachdoc != null">
                    {{tableDataRow.attachdoc}}
                    <%-- <el-input v-model="tableDataRow.attachdoc" :disabled="true"></el-input>--%>
                </el-form-item>
                <el-form-item label="申请时间" v-if="tableDataRow.sqTime != null">
                    {{tableDataRow.sqTime}}
                </el-form-item>
                <el-form-item label="申请状态" v-if="tableDataRow.applyStatus != null">
                    {{tableDataRow.applyStatusValue}}
                </el-form-item>
                <el-form-item label="组长" v-if="tableDataRow.compareRealName != null">
                    {{tableDataRow.compareRealName}}
                </el-form-item>
                <el-form-item label="成员一" v-if="tableDataRow.memberOneRealName != null">
                    {{tableDataRow.memberOneRealName}}
                </el-form-item>
                <el-form-item label="成员二" v-if="tableDataRow.memberTwoRealName != null">
                    {{tableDataRow.memberTwoRealName}}
                </el-form-item>
                <el-form-item label="指导老师" v-if="tableDataRow.teacherRealName != null">
                    {{tableDataRow.teacherRealName}}
                </el-form-item>
                <el-form-item label="驳回意见" v-if="tableDataRow.applyStatus == 5">
                    {{tableDataRow.teacherFinalreJection}}
                </el-form-item>
            </el-form>

            <%--<div slot="footer" class="dialog-footer">
                <el-button  type="primary"  size="mini" @click="outerVisible = false">确 定</el-button>
            </div>--%>
        </el-dialog>
    </template>

    <%--弹出框（提交）--%>
    <template>
        <el-dialog title="" :visible.sync="dialogVisible" class="customClass" top="true" width="80%" center>
            <div slot="title" class="dialog-header">
                显示详细
            </div>

            <el-form ref="tableFormRow" label-width="80px" v-model="tableDataRow" size="mini">
                <el-form-item label="编号">
                    {{tableDataRow.num}}
                </el-form-item>
                <el-form-item label="账号">
                    <el-input v-model="tableDataRow.name"></el-input>
                </el-form-item>
                <el-form-item label="姓名">
                    <el-input v-model="tableDataRow.realName"></el-input>
                </el-form-item>
                <el-form-item label="性别">
                    <el-input v-model="tableDataRow.sex"></el-input>
                </el-form-item>
                <el-form-item label="电话">
                    <el-input v-model="tableDataRow.tel"></el-input>
                </el-form-item>
                <el-form-item label="简介">
                    <el-input v-model="tableDataRow.descInfo"></el-input>
                </el-form-item>
                <el-form-item label="身份">
                    <el-input v-model="tableDataRow.level"></el-input>
                </el-form-item>
                <el-form-item>
                    <el-col :span="24">
                        <el-button type="primary" @click="onSubmit('tableFormRow')">立即提交</el-button>
                        <el-button @click="resetForm('tableFormRow')">重置</el-button>
                    </el-col>
                </el-form-item>
            </el-form>

            <%--<div slot="footer" class="dialog-footer">
                <el-button  type="primary"  size="mini" @click="outerVisible = false">确 定</el-button>
            </div>--%>
        </el-dialog>
    </template>

</div>

<jsp:include page="../vuetemplate/js_vue_template.jsp" flush="true"></jsp:include>
<script type="text/javascript" src="${path}/static/js/jsp/param_user.js"></script>
</body>
</html>
